 ...
 * 移动视觉差对象
 */
const _translateX = function(rootNode, scope, direction, action, speed, nodes, distance) {
    ...
    //直接操作元素
    _transformNodes(rootNode, speed, moveTranslate, offsetTranslate.opacityStart || 0);
}
        ...
        if (baseContents) {
            ...
            _.each(baseContents, function(content) {
                content.eachAssistContents(function(scope) {
                    //如果是视察对象移动
                    if (scope.parallax) {
                        const rootNode = scope.parallax.rootNode;
                        ...
                        //移动视觉差对象
                        _translateX(rootNode, scope.parallax, direction, action, speed, nodes, distance);
                    }
                })
            })
        }
