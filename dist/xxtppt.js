(function (global, factory) {
   ...
   var _set = function name(key, val) {
       var setkey;
   ...
   function _save(name, val) {
       _set(name || TAG, JSON.stringify(val));
   }
   ...
   function setProportion(width, height, left, top) {
       return fiexdProportion$1.apply(this, arguments);
   }
   ...
   */
    ...
    * @return {[type]} [description]
    */
   function loader$1(callback, base) {
       var jsPath,
           cssPath,
           completeCount,
   ...
   function setOption() {
       ...
       iconHeight = config$4.iconHeight;
       proportion$1 = config$4.proportion;
       calculate = proportion$1.calculateContainer();
       TOP = isIOS ? 20 : 0;
       sWidth = calculate.width;
       sHeight = calculate.height;
       proportion$1 = config$4.layoutMode == "horizontal" ? proportion$1.width : proportion$1.height;
       iconHeight = isIOS ? iconHeight : round(proportion$1 * iconHeight);
   }
   ...
   function home() {
       ...
       if (config$4.scrollPaintingMode) {
           retStr = '<div id="sceneHome" style ="width:' + (config$4.virtualMode ? sWidth / 2 : sWidth) + 'px;height:' + sHeight + 'px;top:' + calculate.top + 'px;left:' + calculate.left + 'px;z-index:' + Xut.sceneController.createIndex() + '" class="xut-chapter">';
       } else {
           //overflow:hidden;
           retStr = '<div id="sceneHome" style ="width:' + (config$4.virtualMode ? sWidth / 2 : sWidth) + 'px;height:' + sHeight + 'px;top:' + calculate.top + 'px;left:' + calculate.left + 'px;overflow:hidden;z-index:' + Xut.sceneController.createIndex() + '" class="xut-chapter">';
       }
       ...
       //滑动菜单
       if (config$4.layoutMode == 'horizontal') {
   ...
   function scene(id) {
       ...
       return String.format(wapper, 'scenario-' + id, config$4.virtualMode ? sWidth / 2 : sWidth, sHeight, calculate.top, calculate.left, Xut.sceneController.createIndex(), 'scenarioPage-' + id, 2, 'scenarioMaster-' + id, 1);
   }
   ...
   function initconf(index) {
       ...
       _layoutMode = config$3.layoutMode;
       pageIndex = index;
   }
   ...
   function bindEvent(eventDrop, data) {
       ...
       switch (eventName) {
           ...
           default:
               ...
               eventReference = eventName === 'tap' ? tapEvent(eventContext, eventHandler, data.supportSwipe) : complexEvent(eventContext, eventName, eventHandler, data.supportSwipe);
               break;
       }
   ...
   function createMaster(svgContent, data) {
       ...
       if (imageLayerData) {
           //蒙版图（与背景图是组合关系）
           maskLayer = data["imageMask"] ? prefix$1("mask-box-image") + ":url(" + data.path + data["imageMask"] + ");" : "";
           //图片层
       ...
       if (backImageData) {
           ...
           if (backMaskData) {
               //带蒙版
               if (prefix$1('mask-box-image') != undefined) {
                   restr += '<div class="backImage" style="width:{7};height:100%;position:absolute;z-index:1;background-size:100% 100%;background-image:url(' + data.path + backImageData + ');' + prefix$1('mask-box-image') + ':url(' + data.path + backMaskData + ');{8}"></div>';
               } else {
   ...
   function maskContent(data, wrapObj) {
       ...
       //如果有蒙版图
       isMaskImg = data.mask ? prefix$2('mask-box-image') + ":url(" + Xut.config.pathAddress + data.mask + ");" : "";
   ...
   var playState = prefix$3('animation-play-state');
   //动画前缀
   var prefixAnims = prefix$3('animation');
    ...
    * 计算两点直线距离
    */
   function calculateDistance$1(currentPoint, historyPoint) {
   ...
   function extension(animproto){ //文字动画
   ...
   animproto.getEffectFlicker=function(parameter,object,duration,delay,repeat){if(!("-webkit-filter" in object[0].style))return new TimelineMax(); //if (repeat < 2) repeat = 2; //默认三次
   ...
   var color=colorHexToRGB(color2,opacity);var t1=new TimelineMax({delay:delay,repeat:repeat,onStart:this.startHandler,onStartParams:[parameter,object],onComplete:this.completeHandler,onCompleteParams:[parameter,object,{ //"box-shadow": "none"
   "-webkit-filter":"none"}]});t1.to(object,duration,{onUpdate:updateEffectFlicker});return t1;function updateEffectFlicker(){var progress=t1.progress();var percent=parseInt(progress*(maxSize-minSize));if(progress>0.5)percent=parseInt((1-progress)*(maxSize-minSize)); //object.css("box-shadow", distance + "px " + distance + "px " + minSize + "px " + (minSize + percent) + "px " + color);
   object.css("-webkit-filter","drop-shadow("+color+" "+distance+"px "+distance+"px "+(minSize+percent)+"px)");}}; //跷跷板
   ...
   animproto.getEffectDesaturate=function(parameter,object,duration,delay,repeat){if(!("-webkit-filter" in object[0].style))return new TimelineMax();var saturation=parameter.saturation?parameter.saturation:0.5; //饱和度
   var t1=new TimelineMax({delay:delay,repeat:repeat,onStart:this.startHandler,onStartParams:[parameter,object],onComplete:this.completeHandler,onCompleteParams:[parameter,object,{"-webkit-filter":"none"}]});t1.to(object,duration,{onUpdate:updateSaturate});return t1;function updateSaturate(){var progress=t1.progress();var percent=progress<=0.5?progress*2:1;var val=1+(saturation-1)*percent;object.css("-webkit-filter","saturate("+val+")");}}; //加深
   animproto.getEffectDarken=function(parameter,object,duration,delay,repeat){if(!("-webkit-filter" in object[0].style))return new TimelineMax();var brightness=parameter.brightness&&parameter.brightness<1?brightness.saturation:0.5; //亮度
   var t1=new TimelineMax({delay:delay,repeat:repeat,onStart:this.startHandler,onStartParams:[parameter,object],onComplete:this.completeHandler,onCompleteParams:[parameter,object,{"-webkit-filter":"none"}]});t1.to(object,duration,{onUpdate:updateBrightness});return t1;function updateBrightness(){var progress=t1.progress();var percent=progress<=0.5?progress*2:1;var val=1+(brightness-1)*percent;object.css("-webkit-filter","brightness("+val+")");}}; //变淡
   animproto.getEffectLighten=function(parameter,object,duration,delay,repeat){if(!("-webkit-filter" in object[0].style))return new TimelineMax();var brightness=parameter.brightness&&parameter.brightness>1?parameter.brightness:1.5; //亮度
   var t1=new TimelineMax({delay:delay,repeat:repeat,onStart:this.startHandler,onStartParams:[parameter,object],onComplete:this.completeHandler,onCompleteParams:[parameter,object,{"-webkit-filter":"none"}]});t1.to(object,duration,{onUpdate:updateBrightness});return t1;function updateBrightness(){var progress=t1.progress();var percent=progress<=0.5?progress*2:1;var val=1+(brightness-1)*percent;object.css("-webkit-filter","brightness("+val+")");}}; //透明
   ...
   distance+=calculateDistance$1(quArr[m],quArr[m-1]);sprotInfo.push({start:0,end:distance,quadrant:calculateDirection(quArr[m],quArr[m-1])});} //修改时间为帧数(距离转换为帧)
   ...
   var historyPoint=null;var startEvent=function startEvent(e){historyPoint={x:hasTouch?e.changedTouches[0].pageX:e.clientX,y:hasTouch?e.changedTouches[0].pageY:e.clientY};};var moveEnd=function moveEnd(){historyPoint=null; //松手后行为(辅助对象ID)
   if(parameter.gesture.afterTouch>0)Xut.Assist.Run(parameter.pageType,parameter.gesture.afterTouch,null);};var moveEvent=function moveEvent(e){var i;var currentPoint={x:hasTouch?e.changedTouches[0].pageX:e.clientX,y:hasTouch?e.changedTouches[0].pageY:e.clientY};var d=calculateDistance$1(currentPoint,historyPoint); //鼠示移动距离
   var quadrant1=0; //对象移动方向
   for(i=0;i<sprotInfo.length;i++){if(currentFrame<=sprotInfo[i].end){quadrant1=sprotInfo[i].quadrant;break;}}var quadrant2=calculateDirection(currentPoint,historyPoint); //鼠标移动方向
   ...
   PptAnimation.prototype = {
        ...
        * @return {[type]}               [description]
        */
       _initAnimation: function _initAnimation(startEvent, completeEvent) {
           var self = this;
        ...
        * @return {[type]}       [description]
        */
       _getTimeline: function _getTimeline(data, index) {
        ...
        * @return {[type]} [description]
        */
       _unlockHandler: function _unlockHandler() {
           //购买解锁
        ...
        * @return {[type]}           [description]
        */
       completeHandler: function completeHandler(parameter, object, params) {
        ...
        * @return {[type]}               [description]
        */
       runAnimation: function runAnimation(scopeComplete) {
    ...
    */
   function addQueue(pageIndex, key, value) {
    ...
    * @return {[type]} [description]
    */
   function addRenderer(pageIndex, callback, mark) {
       var uuid = makeGuid();
       if (mark) {
           uuid = uuid + '-' + mark;
       }
    ...
    * @return {[type]} [description]
    */
   function stopRenderer(pageIndex, uuid) {
    ...
    * @return {[type]} [description]
    */
   function destroyRenderer(pageIndex, uuid) {
   ...
   var Factory$1 = Xut.CoreObject.extend({
       ...
       destroyAnim: function destroyAnim() {
           this.action = 'destroy';
    ...
    * 普通精灵动画
    */
   function Sprite$2(option, callback) {
       ...
       if ('png' == imageextension) {
           loader.add(imageFilename, jsonUrl).load(this.load.bind(this));
       } else {
           ...
           loader.add(imageFilename, jsonUrl).add(maskFilename, maskJsonUrl).load(this.load.bind(this));
       }
       loader = null;
   }
   ...
   Sprite$2.prototype.analysisPath = function () {
       var pathImg,
    ...
    * 加载
    */
   Sprite$2.prototype.load = function (loader, res) {
    ...
    * 绘制动画
    */
   Sprite$2.prototype.render = function () {
   ...
   var Sprite$1 = Factory$1.extend({
        ...
        * @return {[type]}               [description]
        */
       constructor: function constructor(successCallback, failCallback, options) {
           this.pageIndex = options.pageIndex;
           //普通精灵动画
           this.sprite = new Sprite$2(options, successCallback);
       },
   ...
   animProto.init = function (id, context, rootNode, chapterId, parameter, pageType) {
       ...
       if (this.canvasMode) {
           ...
           if (actionTypes.spiritId) {
               ...
               this.pixiObj = new Sprite$1(opts);
               //构建精灵动画完毕后
               //构建ppt对象
           ...
           if (actionTypes.compSpriteId) {
               this.pixiObj = new specialSprite(opts);
               //ppt动画
               if (actionTypes.pptId) {
                   self.pptObj = create(PptAnimation, $(self.$contentProcess.view));
               }
   ...
   function Parallax(data) {
       ...
       screenSize$1 = Xut.config.screenSize;
       ...
       //计算出偏移值
       offsetTranslate = conversionValue(translate, nodeOffsetProportion, screenSize$1),
   ...
   function preRunAction(data, eventName) {
       ...
       if (parameter.length === 1) {
           ...
           if (para.animationName === 'EffectAppear' && data.domMode //并且只有dom模式才可以，canvas排除
            && eventName === 'auto' && !para.videoId && !para.delay && data.contentDas.category !== 'Sprite' && !para.preCode //动画前脚本
            && !para.postCode //动画后脚本
   ...
   function extendEvent(activitProto) {
   ...
   **********************************************************************/
   ...
   function extendPrivate(activitProto) {
    ...
    * 文本框
    */
   function extendTextBox(activitProto) {
   ...
   BookMark$1.prototype.createBookMark = function () {
       ...
       var height = sLineHeiht$1 * 3,
           // menu的高为3em
       box = '<div class="xut-bookmark-menu" style="width:100%;height:{0}px;left:0;top:{1}px;">' + '<div class="xut-bookmark-wrap">' + '<div class="xut-bookmark-add">加入书签</div>' + '<div class="xut-bookmark-off" style="background-image:url({2})"></div>' + '<div class="xut-bookmark-view">书签记录</div>' + '</div>' + '</div>' + '<div class="xut-bookmark-list" style="display:none;width:100%;height:{3}px;">' + '<ul class="xut-bookmark-head">' + '<li class="xut-bookmark-back">返回</li>' + '<li>书签</li>' + '</ul>' + '<ul class="xut-bookmark-body"></ul>' + '</div>';
       box = String.format(box, height, sHeight, icons$1.hide, sHeight);
       this.markHeight = height;
       return $(box);
   };
    ...
    * @return {[type]} [description]
    */
   BookMark$1.prototype.closeBookMark = function () {
       this.bookMarkMenu.css({
           transform: 'translate3d(0,0,0)',
           'transition-duration': '1s'
       });
   ...
   * @return {[type]} [description]
   */
   function extendBookMark(activitProto) {
   ...
   SearchBar.prototype.handleEvent = function (evt) {
       ...
       switch (target.className) {
           case 'xut-search-btn':
               ...
               this.isChange = true;
               this.iconManager(icons$2.clear);
               break;
           case 'xut-search-input':
               //实时搜索
               this.search(target.value);
               //还原按钮图标
               this.iconManager(icons$2.search);
               this.isChange = false;
               break;
   ...
   function extendSearchBar(activitProto) {
   ...
   function activityClass(data) {
        ...
        * @type {Array}
        */
       this.abstractContents = Content$1(this);
   ...
   MasterProto._transformNodes = function (rootNode, speed, property, opacityStart) {
       ...
       ////////////////
       if (effect) {
           style[prefix$4('transition-duration')] = speed + 'ms';
           style[prefix$4('transform')] = effect;
           rootNode.css(style);
       }
   };
   ...
   MasterProto._fixPosition = function (filter) {
       ...
       function setPosition(parallaxObj, position) {
           ...
           function toMove(distance, speed) {
               ...
               if (element) {
                   element.css(prefix$4('transition-duration'), speed + 'ms');
                   element.css(prefix$4('transform'), 'translate3d(' + distance + 'px,0px,0px)');
               }
           }
   ...
   function Manager(parameter) {
       ...
       //创建翻页滑动
       var $globalEvent = vm.$globalEvent = new GlobalEvent(options, config$7);
   ...
   portExtend(Application, {
       ...
       DropApp: function DropApp() {
           ...
           function unEvent() {
               ...
               //安卓销毁按键事件
               if (Xut.plat.isAndroid) {
                   window.GLOBALCONTEXT.document.removeEventListener("backbutton", config$2._event.back, false);
                   window.GLOBALCONTEXT.document.removeEventListener("pause", config$2._event.pause, false);
               }
           }
   ...
   function fixedSize(novelData) {
       if (novelData) {
           if (novelData.pptWidth || novelData.pptHeight) {
               config$1.setProportion(novelData.pptWidth, novelData.pptHeight);
           }
       }
   }
