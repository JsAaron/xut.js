...
define("Store", [
	...
	Store.oneQuery = function(tableName,callback) {
		ExecuteSql('select * FROM ' + tableName + ' order by _id ASC', function(sqlRet, collectError) {
			callback(sqlRet);
		})
	}
	...
	Store.query = function() {
		...
		return $.Deferred(function(dfd) {
			...
			if(Xut.IBooks.CONFIG){
				...
				//查询所有数据
				ExecuteSql(Store.statement, function(sqlRet, collectError) {
					self.collectError = collectError;
					for (i in sqlRet) {
						dataRet[i] = sqlRet[i];
					}
	...
	Store.remove = function(tableName, id){
		...
		return $.Deferred(function(dfd) {
			//查询所有数据
			ExecuteSql(sql, function(success, failure) {
				if (success) { //成功回调
					dfd.resolve();
